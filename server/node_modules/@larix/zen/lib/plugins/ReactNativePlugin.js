...
var resolveModule_1 = require("./shared/resolveModule");
var UPFinder_1 = require("./shared/UPFinder");
var babelRegisterDone = false;
var registerBabel = function (builder) {
    if (!babelRegisterDone) {
        var babelPkgJson = builder.require.probe('babel-core') && builder.require('babel-core/package.json');
        var isBabel7 = builder.require.probe('@babel/core') && (!babelPkgJson || babelPkgJson.version.indexOf('bridge') >= 0);
...
var ReactNativePlugin = (function () {
    ...
    ReactNativePlugin.prototype.configure = function (builder, zen) {
        ...
        if (stack.hasAll(['react-native', 'webpack'])) {
            ...
            var babelrc = new UPFinder_1.default(builder).find(['.babelrc.native', 'babel.config.native.js']);
            var jsRuleFinder_1 = new JSRuleFinder_1.default(builder);
            var jsRule = jsRuleFinder_1.findJSRule();
            if (jsRule) {
                jsRule.exclude = function (modulePath) {
                    var moduleType = resolveModule_1.default(builder, modulePath).moduleType;
                    var result = moduleType === resolveModule_1.ModuleType.NormalNodeModule || moduleType === resolveModule_1.ModuleType.TranspiledNodeModule;
                    return result;
                };
            }
            var cacheDirectory = builder.cache === false || (builder.cache === 'auto' && !zen.dev)
                ? false
                : path.join(builder.cache === true || (builder.cache === 'auto' && zen.dev) ? '.cache' : builder.cache, 'babel-loader');
            var babelPkgJson = builder.require.probe('babel-core') && builder.require('babel-core/package.json');
            var isBabel7 = builder.require.probe('@babel/core') && (!babelPkgJson || babelPkgJson.version.indexOf('bridge') >= 0);
            var reactNativePreset = isBabel7 && builder.require.probe('metro-react-native-babel-preset')
                ? 'module:metro-react-native-babel-preset'
                : 'react-native';
            var defaultConfig = babelrc
                ? JSON.parse(fs.readFileSync(babelrc).toString())
                : {
                    compact: !zen.dev,
                    presets: [reactNativePreset],
                    plugins: ['haul/src/utils/fixRequireIssues']
                };
            var babelOptions = zen.createConfig(builder, 'babel', __assign({ babelrc: false, configFile: false, cacheDirectory: cacheDirectory }, defaultConfig));
            builder.config.module.rules.push({
                test: /\.js$/,
                exclude: function (modulePath) {
                    var result = resolveModule_1.default(builder, modulePath).moduleType !== resolveModule_1.ModuleType.TranspiledNodeModule;
                    return result;
                },
                use: {
